...
---
= TR-5007: Optimice la copia de seguridad y la recuperación de Oracle VLDB con la fusión incremental en el servidor en Google Cloud NetApp Volumes
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Recuperar una base de datos muy grande (VLDB) en Oracle mediante la herramienta de respaldo Oracle Recovery Manager (RMAN) puede ser una tarea muy desafiante.  El proceso de restauración de la base de datos desde medios de respaldo en caso de falla puede consumir mucho tiempo, retrasar la recuperación de la base de datos y potencialmente afectar significativamente su Acuerdo de Nivel de Servicio (SLA).  La situación está empeorando aún más para Oracle VLDB implementado en la nube, ya que la copia de seguridad de RMAN generalmente se transmite al almacenamiento de objetos debido a consideraciones de costos.  Como resultado, no es viable restaurar y recuperar un VLDB si el SLA es esencial.  A partir de la versión 10g, Oracle introdujo una función RMAN que permite a los usuarios crear copias de imágenes preparadas de los archivos de datos de la base de datos Oracle en un almacenamiento de disco adicional ubicado en el host del servidor de base de datos.  Estas copias de imágenes se pueden actualizar de forma incremental mediante RMAN en un intervalo definido por el usuario.  En caso de falla, el administrador de base de datos (DBA) puede cambiar rápidamente la base de datos Oracle del medio fallido a la copia de imagen, eliminando la necesidad de una restauración completa del medio de la base de datos.  El resultado es un SLA muy mejorado, aunque a costa de duplicar el almacenamiento de base de datos requerido.

Si está interesado en el SLA para su VLDB y está considerando trasladar la base de datos Oracle a una nube pública como Google Cloud Platform (GCP), puede configurar una estructura de protección de base de datos similar utilizando recursos como Google Cloud NetApp Volumes (GCNV) para preparar su copia de imagen de base de datos en espera.  GCNV es un servicio de almacenamiento de archivos de alto rendimiento y totalmente administrado ofrecido por Google que hace que sea más fácil, seguro y rentable migrar y ejecutar aplicaciones empresariales exigentes, como la base de datos Oracle, en Google Cloud.  Sus variados niveles de servicio permiten la flexibilidad de controlar el costo de almacenamiento mientras se logra un RPO cercano a cero y un RTO manejable.  En esta documentación, demostramos cómo aprovisionar y exportar un sistema de archivos NFS desde un grupo de almacenamiento GCNV para montarlo en un servidor de base de datos Oracle para almacenar temporalmente una copia de base de datos en espera para una recuperación rápida en caso de una falla del almacenamiento principal.
...
Las pruebas y la validación de esta solución se realizaron en grupos de almacenamiento de Google Cloud NetApp Volumes y máquinas virtuales de Google Compute Engine que podrían no coincidir con el entorno de implementación final. Para más información, consulte la sección <<Factores clave a considerar en la implementación>> .



=== Arquitectura

image:gcp-ora-gcnv-inc-merge-architecture.png["Esta imagen proporciona una imagen detallada de la implementación de la fusión incremental de Oracle VLDB en Google Cloud con GCNV."]
...
* *Nivel de servicio del grupo de almacenamiento GCNV.*  El grupo de almacenamiento GCNV ofrece cuatro niveles de servicio: Estándar, Premium, Extremo y Flex.  De forma predeterminada, se aplica una QoS automática a un volumen creado dentro de un grupo de almacenamiento, lo que restringe el rendimiento del volumen en función de su tamaño y del nivel de servicio.  La única excepción es que los volúmenes asignados desde el grupo de almacenamiento Flex comparten el rendimiento disponible para el grupo de almacenamiento.
* *Límite de almacenamiento de respaldo en servidor Oracle VLDB para la fusión incremental de RMAN.*  En nuestra prueba y validación, se asigna un volumen NFS para la copia de seguridad y fusión incremental de Oracle desde un único grupo de almacenamiento GCNV con un nivel de servicio estándar, que tiene un límite de capacidad de 200 TiB.  Para implementaciones que superen esos umbrales, se pueden implementar múltiples grupos de almacenamiento GCNV en paralelo con múltiples puntos de montaje NFS para proporcionar mayor capacidad.
* *Recuperabilidad de Oracle mediante la fusión incremental de RMAN.*  La copia de seguridad y la combinación incrementales en el servidor RMAN generalmente se ejecutan con una frecuencia definida por el usuario en función de sus objetivos de RTO y RPO.  Si hay una pérdida total del almacenamiento de archivos de la base de datos primaria y/o de los registros archivados, puede producirse una pérdida de datos.  La base de datos de Oracle se puede recuperar hasta la última copia de seguridad incremental disponible desde la copia de imagen de copia de seguridad de la base de datos en el servidor.  Para minimizar la pérdida de datos, el área de recuperación flash de Oracle se debe configurar en el punto de montaje de respaldo NFS de GCNV, y los registros archivados se respaldan en el mismo montaje NFS junto con la copia de la imagen de la base de datos para revertir la base de datos al último registro archivado, en caso de una falla.  De esta forma se puede conseguir un RPO cercano a cero.
* *Ejecución de Oracle VLDB desde una copia de seguridad de imagen NFS en el grupo de almacenamiento GCNV.*  En este punto, no se admite la reasignación del nivel de servicio del volumen de base de datos de estándar a premium o extremo.  Sin embargo, se admite la reasignación de niveles de servicio de premium a extremo.  Para ejecutar la base de datos en una copia de imagen durante un período prolongado después de una falla, se recomienda asignar un volumen del grupo de almacenamiento premium para la copia de seguridad de la imagen de la base de datos.
* *Máquinas virtuales de Google Compute Engine.*  En la prueba y validación, utilizamos el tipo N1 de instancia de VM GCE como servidores de base de datos de Oracle.  Hay otros tipos de máquinas virtuales GCE que pueden estar más optimizadas y ser más adecuadas para la carga de trabajo de la base de datos.  Dimensione la máquina virtual adecuadamente para la cantidad de vCPU y la cantidad de RAM en función de los requisitos de carga de trabajo reales.
* *Configuración de dNFS.* dNFS está integrado en el kernel de Oracle y se sabe que aumenta drásticamente el rendimiento de la base de datos de Oracle cuando Oracle se implementa en el almacenamiento NFS. dNFS está empaquetado en el binario de Oracle, pero no está activado de forma predeterminada.  Debe estar activado para cualquier implementación de base de datos Oracle en NFS.  Para la implementación de múltiples grupos de almacenamiento GCNV para un VLDB, las rutas múltiples de dNFS a diferentes grupos de almacenamiento GCNV se deben configurar correctamente.
...
La solución supone que ya hay un Oracle VLDB implementado en el entorno de Google Cloud dentro de una VPC.  Oracle VLDB puede ejecutarse en el almacenamiento GCNV o en cualquier otra opción de almacenamiento dentro del ecosistema de Google Cloud.  La siguiente sección proporciona procedimientos de implementación paso a paso para configurar una copia de seguridad de imagen de Oracle VLDB y una fusión incremental a través de RMAN a un sistema de archivos NFS montado desde un grupo de almacenamiento GCNV.  Oracle VLDB también tiene su almacenamiento principal alojado en un grupo de almacenamiento GCNV diferente.
...
. Desde la consola del portal de Google Cloud, siga la documentaciónlink:https://cloud.google.com/netapp/volumes/docs/configure-and-use/storage-pools/create-storage-pool["Crear un grupo de almacenamiento"^] para crear un grupo de almacenamiento GCNV con servicio estándar como se muestra a continuación.
+
image:gcp-ora-gcnv-inc-merge-pool-001.png["Esta imagen muestra la pantalla del grupo de almacenamiento GCNV desde la consola del portal de Google Cloud."]

. Desde la consola del portal de Google Cloud, siga la documentaciónlink:https://cloud.google.com/netapp/volumes/docs/configure-and-use/volumes/create-volume["Crear un volumen"^] para crear un volumen para la copia de seguridad de la imagen Oracle VLDB en el servidor.  En esta demostración de solución, el almacenamiento principal de Oracle VLDB también está alojado en GCNV como se muestra a continuación.
+
image:gcp-ora-gcnv-inc-merge-vol-002.png["Esta imagen muestra la pantalla de volúmenes de almacenamiento de GCNV desde la consola del portal de Google Cloud."]
...
+
image:gcp-ora-gcnv-inc-merge-vol-003.png["Esta imagen muestra la pantalla de volúmenes de almacenamiento de GCNV desde la consola del portal de Google Cloud."]

. Inicie sesión en el servidor Oracle VLDB principal.  Como root, monte el volumen NFS exportado desde el almacenamiento GCNV.  Cambie la dirección IP de su servidor NFS y la ruta del archivo según sea necesario.
+
[source, cli]
----
sudo mkdir /nfsgcnv
...
[%collapsible%open]
====
La combinación incremental de RMAN actualiza la copia de imagen de los archivos de datos de la base de datos de ensayo de forma continua en cada intervalo de combinación/copia de seguridad incremental.  La copia de la imagen de la copia de seguridad de la base de datos estará tan actualizada como la frecuencia con la que ejecute la copia de seguridad/combinación incremental.  Por lo tanto, tenga en cuenta el rendimiento de la base de datos y sus objetivos de RTO y RPO al decidir la frecuencia de las copias de seguridad incrementales y la fusión de RMAN.

. Inicie sesión en el servidor Oracle VLDB principal como usuario Oracle.
. Para esta validación de la solución, la base de datos principal se configura como un servidor independiente con infraestructura de red. Reinicie la configuración como se muestra a continuación.  El almacenamiento principal está alojado en el grupo de almacenamiento GCNV con servicio premium.  Nuevamente, esto no es un requisito y el almacenamiento principal puede estar en cualquier otra opción de almacenamiento en la nube de Google.
+
....
...
----
. Inicie sesión en la base de datos de Oracle a través de sqlplus, habilite el seguimiento de cambios de bloque para una copia de seguridad incremental más rápida y cambie el área de recuperación flash de Oracle al montaje NFS GCNV si actualmente está en el almacenamiento principal.  Esto permite que el archivo de control predeterminado de RMAN, la copia de seguridad automática del archivo/spfile y los registros archivados se respalden en el montaje NFS de GCNV para la recuperación de la base de datos al último registro de archivo disponible.
+
[source, cli]
----
sqlplus / as sysdba
...
....
. Enumere la copia de imagen de la base de datos después de la copia de seguridad para observar que se ha creado una copia de imagen de la base de datos en el punto de montaje NFS de GCNV.
+
....
...
[%collapsible%open]
====
En caso de una falla debido a un problema de almacenamiento primario, como pérdida o corrupción de datos, la base de datos se puede cambiar rápidamente a una copia de imagen en un montaje NFS GCNV y recuperar al estado actual sin restaurar la base de datos.  La eliminación de la restauración de medios acelera enormemente la recuperación de la base de datos para una VLDB.  Este caso de uso supone que el servidor de base de datos Oracle VLDB está intacto y que el archivo de control de la base de datos y los registros archivados y actuales están todos disponibles para la recuperación.
...
....
. Insertar una fila en la tabla de prueba.  La fila se inserta después de una copia de seguridad incremental y una combinación más recientes y sus datos no están en el conjunto de copias de seguridad.  La demostración posterior mostrará que la fila de prueba se recupera después de que la base de datos se recupera por completo.
+
....
...
....
. Verifique la estructura de la base de datos desde sqlplus después de la recuperación para observar que todos los archivos de datos VLDB, con excepción de los archivos de control, temporales y de registro actuales, ahora se cambian para copiar en el sistema de archivos NFS GCNV.
+
....
...
[%collapsible%open]
====
Una vez corregida la falla de almacenamiento en el almacenamiento primario, la base de datos se puede revertir fácilmente al almacenamiento primario con un tiempo de inactividad mínimo.  A continuación se detallan los procedimientos paso a paso.
...
....
. Revise el script de copia de seguridad y combinación de rman para que apunte al almacenamiento principal y, en nuestro caso de prueba, al grupo de discos ASM +DATA en la definición del canal RMAN.  Ejecute el script de respaldo revisado para crear una nueva copia de imagen en el almacenamiento principal.
+
....
...
[%collapsible%open]
====
En el caso de una falla donde se pierda el almacenamiento primario y/o el servidor de base de datos primario, se perderán todas las transacciones que no estén en los archivos de registro archivados.  Sin embargo, la copia de seguridad de la imagen de la base de datos Oracle disponible en el sistema de archivos NFS GCNV resulta útil.  Puede recuperar rápidamente la base de datos principal en el mismo servidor de base de datos principal o en un servidor de base de datos en espera, si hay uno disponible, utilizando la copia de imagen de respaldo.  En esta sección, demostraremos los procedimientos paso a paso para la recuperación al servidor de base de datos principal.  Los métodos para recuperarse en un nuevo servidor de base de datos en espera son casi idénticos; sin embargo, la copia de la imagen de respaldo de la base de datos debe montarse en el servidor de base de datos en espera con la misma pila de software Oracle instalada y configurada.
...
. Luego, la copia de seguridad de la imagen de la base de datos Oracle en el sistema de archivos NFS GCNV se monta en el servidor de base de datos en espera para su recuperación.  Los siguientes procedimientos demuestran los detalles del proceso.
+
...
----
. Verifique los registros archivados de Oracle disponibles en el montaje NFS de GCNV para la recuperación y anote el número de secuencia de registro del último archivo de registro.  En nuestro caso de prueba, fue 27.  Nuestro punto de recuperación está hasta la secuencia de registro número 28.
. Si se recupera de un host de base de datos en espera, como usuario de Oracle, configure la variable ORACLE_HOME en la instalación actual de Oracle en el servidor de base de datos en espera y ORACLE_SID en el SID de la instancia principal de Oracle.  En esta prueba, es NTAP.
+
....
...
....
. Como usuario de Oracle, cree un archivo de inicio de Oracle genérico en el directorio $ORACLE_HOME/dbs con los directorios de administración adecuados configurados.  Lo más importante es tener Oracle `flash recovery area` apunta a la ruta de montaje NFS de GCNV tal como se define en el servidor Oracle VLDB principal.  `flash recovery area` La configuración se demuestra en la sección `Setup Oracle RMAN incremental merge to image copy on GCNV for in-server VLDB backup` .  Establezca el archivo de control de Oracle en el sistema de archivos de respaldo NFS GCNV.
+
[source, cli]
----
...
....
. Establecer ID de base de datos.  La ID de la base de datos se puede recuperar del nombre del archivo Oracle de la copia de la imagen en el punto de montaje NFS de GCNV.
+
....
...
....
. Para volver al almacenamiento principal, siga exactamente los mismos procedimientos que se detallan en la sección `Reverse VLDB back to primary storage from backup copy` .  Si se realiza la recuperación en un servidor de base de datos en espera, el almacenamiento principal debe importarse al servidor de base de datos en espera.  También es necesario reubicar los archivos de control de Oracle y los archivos de registro en línea actuales desde el montaje NFS de GCNV al almacenamiento principal, lo que se puede lograr con unos pocos minutos de tiempo de inactividad.


Esto completa la recuperación en un punto determinado del tiempo de la base de datos Oracle VLDB desde la copia de la imagen de respaldo en el sistema de archivos NFS de GCNV al mismo servidor de base de datos principal o a un servidor de base de datos en espera.
...
[%collapsible%open]
====
Clonar una nueva Oracle VLDB a partir de la copia de imagen de respaldo en GCNV es similar a la recuperación en un punto en el tiempo de la base de datos principal a un nuevo servidor de base de datos.  El equipo de ingeniería de soluciones de NetApp proporciona un kit de herramientas de automatización para facilitar todos los procesos para una implementación rápida (en minutos) y sin esfuerzo.

El siguiente kit de herramientas de automatización contiene un manual que se puede utilizar para crear o actualizar clones de un Oracle VLDB a partir de una copia de imagen de respaldo de la base de datos principal almacenada en el almacenamiento GCNV a través del montaje NFS para una gestión completa del ciclo de vida de los clones.  Para más detalles, consulte la documentación:link:automation-ora-bkup-clone.html["Ciclo de vida automatizado de clones de Oracle VLDB con copia de seguridad de imágenes en el servidor"] .
